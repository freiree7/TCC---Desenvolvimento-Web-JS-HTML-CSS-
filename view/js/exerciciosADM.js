...
function filtroTexto() {
    ...
    for (let exercicio of exercicios_json) {
        ...
        if (textoBusca && nome.includes(textoBusca)) {
            ...
            bookmarkInput.addEventListener("change", (event) => {
                if (event.target.checked) {

                    console.log(`Exercício ${exercicio.nome} adicionado aos favoritos.`);
                    const idexercicio = exercicio.idexercicio;

                    const novaEscolha = {
                        exercicioId: idexercicio
                    }
                    fetch_exercicios_fav(novaEscolha, usuarioId);

                } else {
                    console.log(`Exercício ${exercicio.nome} removido dos favoritos.`);
                    const exercicioid = exercicio.idexercicio;

                    fetch_exercicios_fav_remover(exercicioid);
